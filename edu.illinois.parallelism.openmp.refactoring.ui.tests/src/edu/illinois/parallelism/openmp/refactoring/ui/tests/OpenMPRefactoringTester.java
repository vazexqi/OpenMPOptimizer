 ...
 *     nchen (UIUC) - Made this test suite load resources outside this bundle
 *******************************************************************************/
package edu.illinois.parallelism.openmp.refactoring.ui.tests;
...
/**
 * This is based off
 * {@link org.eclipse.cdt.ui.tests.refactoring.RefactoringTester}. The main
 * reason we have duplicated code here is because of the way the classloader
 * works in createTestClass. If we don't have a call to it here, the classloader
 * is not able to find classes in our bundle.
 * 
 * N.B. There might be another way to abstract away the classloader but this
 * copy-paste method was the easiest to implement for the time being.
 * 
 * @author Emanuel Graf
 * @author nchen
 */
public class OpenMPRefactoringTester extends TestSuite {
	enum MatcherState {
	...
	private static RefactoringBaseTest createTestClass(String className, String testName, Collection<TestSourceFile> files)
			throws Exception {
		try {
	...
	static Bundle bundleForResources = null;

	public static Test suite(String name, String file, Bundle bundle) throws Exception {
		bundleForResources = bundle;
		BufferedReader in = createReader(file);
	...
	protected static BufferedReader createReader(String file) throws IOException {
		Bundle bundle = getBundleForResources();
		Path path = new Path(file);
		String finalFilePath = FileLocator.toFileURL(FileLocator.find(bundle, path, null)).getFile();
		return new BufferedReader(new FileReader(finalFilePath));
	}

	private static Bundle getBundleForResources() {
		return bundleForResources != null ? bundleForResources : Activator.getDefault().getBundle();
	}
}
